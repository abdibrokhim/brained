...
import { faAdd, faChevronDown, faClose, faFile, faChevronLeft, faChevronRight, faCompass, faShare, faPrint, faUpRightAndDownLeftFromCenter, faExpand, faSquareCheck, faTrash, faCircleCheck, faClone, faPenNib, faSquareMinus, faMinus, faCircleInfo, faEye, faBrain, faBars, faTimes } from '@fortawesome/free-solid-svg-icons';
        ...
        <div className="w-full max-w-2xl h-[600px] overflow-auto mx-4 bg-[#2e2e2e] rounded-md">
          ...
          <div className="p-4">
            ...
            <div className="flex flex-row justify-between border border-[#a1a1aa] p-4 rounded-md">
              <p className="text-md">
                Workspace:{' '}
                <span className="text-md ml-[20px] text-[#aaaaaa]">
            ...
            <div className="mt-8 flex flex-col md:flex-row justify-between gap-8">
              {/* Upload scans */}
              <div className="flex flex-col justify-between border border-[#a1a1aa] rounded-md w-full md:w-1/2">
                      ...
                      >
                        <div className="flex flex-col items-center justify-center">
                          ...
                          <p className="text-xs text-[#aaaaaa]">
                            (Only DICOM, PNG, JPEG files supported)
                          </p>
                        </div>
                      </label>
                    </div>
                  ) : (
                    <div className="flex flex-col gap-4 items-center justify-center w-full overflow-auto p-2">
                      {droppedFiles.map((file) => (
                        <div key={file.name} className="flex flex-row gap-2 items-center justify-between w-full">
                          <div className="flex flex-col items-start justify-start p-2 border border-[#a1a1aa] rounded-md w-full">
              ...
              <div className="w-full md:w-1/2">
                {/* Patient ID */}
                <div className="mb-4">
                  <p className="block text-white text-xs font-bold mb-2">Patient ID</p>
                  <input
                    value={patientId}
                    onChange={(e) => {
                ...
                <div className="flex flex-col md:flex-row mb-4">
                  ...
                  <div className="w-full md:w-2/3 md:pl-2">
                    <p className="block text-white text-xs font-bold mb-2">Phone Number</p>
                    <input
                      value={patientPhoneNumber}
...
return (
  <main className="flex min-h-screen flex-col lg:flex-row p-0 relative text-white">
  {/* Show notification */}
  {notification && (
    <Notification
  ...
  {/* Sidebar */}
  <aside className="hidden lg:block lg:w-2/12 bg-[#151515]">
    <div className="flex flex-col items-start justify-start h-full space-y-8">
      ...
      <div className="mt-[100px] ml-[20px] flex flex-col gap-4">
        ...
        <div>
          ...
          >
            <FontAwesomeIcon icon={faAdd} className="mr-2" /> Upload Scan(s)
          </button>
        </div>
        {/* Observations View */}
        <div>
          ...
          >
            <FontAwesomeIcon icon={faCompass} className="mr-2" /> Observations
          </button>
        </div>
        {/* Reports View */}
        <div>
          ...
          >
            <FontAwesomeIcon icon={faFile} className="mr-2" /> Reports
          </button>
        </div>
      </div>
      {/* Account Settings */}
  ...
  <div className="flex-1 relative">
    {/* Header with menu icon on mobile */}
    <div className="flex items-center justify-between p-4 lg:hidden bg-[#151515] absolute top-0 left-0 sticky z-40">
    ...
    {/* Mobile menu */}
    {isMenuOpen && (
      <div className="absolute top-0 left-0 w-full bg-[#151515] z-40 p-4 sticky">
        {/* Menu options */}
        <div className="flex flex-col space-y-4 mt-4">
          <button
            onClick={() => { handleShowUploadImageView(); setIsMenuOpen(false); }}
            className={`text-md cursor-pointer w-full hover:underline border border-white rounded-md px-4 py-4 flex items-center ${showUploadImageView ? 'underline' : ''}`}
          >
            <FontAwesomeIcon icon={faAdd} className="mr-2" /> Upload Scan(s)
          </button>
          <button
            onClick={() => { handleShowPortalView(); setIsMenuOpen(false); }}
            className={`text-md cursor-pointer w-full hover:underline border border-white rounded-md px-4 py-4 flex items-center ${showPortalView ? 'underline' : ''}`}
          >
            <FontAwesomeIcon icon={faCompass} className="mr-2" /> Observations
          </button>
          <button
            onClick={() => { handleShowReportView(); setIsMenuOpen(false); }}
            className={`text-md cursor-pointer w-full hover:underline border border-white rounded-md px-4 py-4 flex items-center ${showReportView ? 'underline' : ''}`}
          >
            <FontAwesomeIcon icon={faFile} className="mr-2" /> Reports
          </button>
